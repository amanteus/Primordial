...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupSalesPage() {
        ...
        const mural = document.getElementById('alcateia-mural');
            if (!mural) return; // Só roda se o mural existir

            const allPosts = [
                { username: 'Ricardo M.', text: "Gente, preciso compartilhar. Pela primeira vez na vida eu discordei da minha chefe numa reunião, com calma, com argumentos, sem gaguejar. Ela não só aceitou, como me olhou com um respeito diferente. O módulo 2 sobre a arte do respeito silencioso... surreal." },
                { username: 'Lobo_Alfa_7', text: "Aquele 'nó na garganta' que eu sentia antes de abordar uma mulher? Sumiu. Não sei explicar. É como se o medo da rejeição simplesmente se tornasse irrelevante. Consegui o número daquela morena da academia hoje. Coisa que eu tava enrolando pra fazer há 6 meses." },
                { username: 'Fernando S.', text: "O mais louco não é nem as mulheres. É como outros homens mudaram o jeito que me tratam. No trabalho, na roda de amigos. A postura muda, a voz muda, e o mundo responde. Assustador e poderoso." },
                { username: 'Bruno C.', text: "Alerta de resultado: 3 semanas no Primordial. Tinha uma conversa empacada no WhatsApp. Usei o princípio da 'imprevisibilidade' do módulo 3. A conversa que tava morta virou um encontro marcado pra sexta. Simples assim." },
                { username: 'Marcos T.', text: "Eu era o 'bom moço'. O terapeuta das minhas amigas. Depois de executar o 'Ritual de Ruptura' do módulo 1, uma amiga que sempre me contava dos caras que ela saía me olhou diferente e disse 'nossa, você tá... diferente'. O jogo virou." },
                { username: 'Tiago P.', text: "Pra quem tá na dúvida: o dinheiro que eu gastava com bebida no fim de semana pra tentar 'criar coragem' já pagou o Primordial. Só que a coragem que eu tenho agora não passa na segunda-feira. Pensem nisso." },
                { username: 'André V.', text: "O bônus do 'Mapa da Neurodominância' sozinho já vale o investimento. Entender o ciclo da dopamina e como o celular e a pornografia estavam me transformando num zumbi apático... isso abriu meus olhos de um jeito que não tem volta." },
            ...
            // Função para gerar uma data fixa baseada no nome
            function generateConsistentDate(name) {
                let hash = 0;
                for (let i = 0; i < name.length; i++) {
                    hash = name.charCodeAt(i) + ((hash << 5) - hash);
                }
                const daysAgo = (Math.abs(hash) % 20) + 2; // de 2 a 26 dias
                const date = new Date();
            ...
            function addPostToMural(post, isNew = false) {
            ...
            let remainingPosts = allPosts.slice(5);
            shuffleArray(remainingPosts);
            let newPostIndex = 0;

            // Popula com os posts iniciais
            initialPosts.forEach(post => addPostToMural(post, false));

            // Agenda a aparição dos novos posts
            function scheduleNextPost() {
                // Frequência reduzida para não competir com as notificações de compra
                const randomDelay = Math.random() * (80000 - 35000) + 35000; // Entre 35s e 80s
                
                setTimeout(() => {
                    if (newPostIndex < remainingPosts.length) {
                        addPostToMural(remainingPosts[newPostIndex], true);
                        newPostIndex++;
                        scheduleNextPost();
                    }
                }, randomDelay);
