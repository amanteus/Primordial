// ==========================================================
// ARQUIVO SCRIPT.JS COMPLETO E INTEGRADO (VERSÃO FINAL 7.0)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // -------------------------------------------------------------------
    // --- MÓDULO 3: SISTEMA DE ESCASSEZ E PROVA SOCIAL (FASE 2) ---
    // -------------------------------------------------------------------
    function initScarcityAndSocialProof() {
        ...
        const simularVenda = () => {
            ...
            setTimeout(() => {
                ...
                const username = comprador.name.split(' ')[0] + '.' + comprador.name.split(' ')[1].charAt(0);
                showNotification(`<span class="notification-name">${username}</span> está ativo na comunidade.`);
            }, Math.random() * (15000 - 8000) + 8000); // Delay de "cadastro"
        };
        ...
        activeMembersCount = TOTAL_VAGAS - vagasAtuais + 20; // Inicia membros ativos
        activeMembersElement.textContent = `+${activeMembersCount}`;
        ...
        const scheduleNextSale = () => {
            const randomDelay = Math.random() * (75000 - 40000) + 40000; // Entre 40 e 75 segundos
            setTimeout(() => {
                simularVenda();
                scheduleNextSale();
            }, randomDelay);
        ...
        setTimeout(scheduleNextSale, 20000); // Primeira venda simulada após 20s
    }
    ...
    function initCommentSystem() {
        ...
        if (!listElement || !template || !filterContainer) return;
        ...
        const renderMural = (filter) => {
            ...
            if (filter === 'relevantes') {
                ...
                comments = comments
                    .filter(c => Date.now() - new Date(c.timestamp).getTime() < 24 * 3600 * 1000)
                    .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
            }
            if(comments.length === 0) listElement.innerHTML = `<p class="no-comments-message">Nenhum comentário para este filtro.</p>`;
            else comments.forEach(c => listElement.appendChild(createCommentEl(c)));
        };
        ...
        const scheduleNewComments = (timeSince) => {
            ...
            const toShow = unseen.sort(() => 0.5 - Math.random()).slice(0, Math.floor(Math.random() * 3) + 2); // 2 a 4 novos
            let delay = 15000;
            toShow.forEach(comment => {
                setTimeout(() => {
                    const seen = getFromStorage(SEEN_KEY) || {};
                    seen[comment.id] = Date.now();
        ...
        renderMural(activeFilter);
        scheduleNewComments(Date.now() - lastVisit);
