...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * MÓDULO 2: WIDGET "ECOS DA ALCATEIA"
     * Controla o carrossel de prova social com persistência.
     */
    function initTweetCarousel() {
        const track = document.querySelector('.carousel-track');
        const indicatorsContainer = document.querySelector('.carousel-indicators');
        const prevButton = document.getElementById('prev-eco');
        const nextButton = document.getElementById('next-eco');

        if (!track || !indicatorsContainer || !prevButton || !nextButton) return;

        const LIKED_KEY = 'amanteus_liked_ecos';
        const SAVED_KEY = 'amanteus_saved_ecos';

        const ecos = [
            { id: 1, name: 'Ricardo S.', handle: '@ricardo_s88', avatar: 'img/avatar1.png', text: 'Mudei minha postura como o Módulo 2 ensina. No dia seguinte, na reunião, todos pararam pra me ouvir. Sinistro.' },
            { id: 2, name: 'Fernando M.', handle: '@fermaciel', avatar: 'img/avatar2.png', text: 'A "paralisia na conversa" era o que mais me matava. O Manual de Interação não é sobre o que dizer, é sobre COMO pensar. Jogo virou.' },
            { id: 3, name: 'Lucas G.', handle: '@lucas_g92', avatar: 'img/avatar3.png', text: 'O Dashboard do Soberano não é uma lista de tarefas, é uma arma contra a hesitação. Transformou meus planos em ação.' },
            { id: 4, name: 'Bruno C.', handle: '@bruno_costa', avatar: 'img/avatar4.png', text: 'A comunidade (Alcateia) é o verdadeiro diferencial. É um campo de batalha onde todo mundo se apoia pra vencer.' },
            { id: 5, name: 'Guilherme T.', handle: '@gui_toscano', avatar: 'img/avatar5.png', text: 'Entendi que eu não precisava ser um "personagem". Apenas remover a programação que me bloqueava. O Primordial instalou o sistema operacional certo.' }
        ];
        ...
        // Popula o carrossel e os indicadores
        ecos.forEach((eco, index) => {
            const isLiked = !!likedEcos[eco.id];
            const isSaved = !!savedEcos[eco.id];
            let likeCount = (eco.id * 17 + 23) % 150 + 15; // Gera um número de likes "realista"
            if(isLiked) likeCount++;

            track.innerHTML += `
                <article class="eco-card" data-eco-id="${eco.id}">
                    <header class="eco-header">
                        <img src="${eco.avatar}" alt="Avatar de ${eco.name}" class="eco-avatar">
                        <div class="eco-user">
