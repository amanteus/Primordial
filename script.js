...
const fakePurchases = [
    ...
    // --- SEÇÃO DE COMENTÁRIOS COM DATA CONSISTENTE ---
        ...
        if (commentsList) {
            ...
            function addPrimordialComment(comment, isNew = false) {
                const commentDiv = document.createElement('div');
                commentDiv.className = 'primordial-comment-item';
                const timeAgo = isNew ? 'há poucos segundos' : formatTimeAgo(generateConsistentDate(comment.name));
                ...
                if (isNew) {
                    ...
                    requestAnimationFrame(() => {
                        ...
                        setTimeout(() => commentDiv.classList.add('is-visible'), 50);
                    });
                } else {
            ...
            initialComments.forEach(c => addPrimordialComment(c, false));
            ...
            function scheduleNextComment() {
                ...
                setTimeout(() => {
                    if (newCommentIndex < 5 && newCommentIndex < remainingComments.length) {
                        addPrimordialComment(remainingComments[newCommentIndex], true);
                        newCommentIndex++;
                        scheduleNextComment();
                    }
                }, randomDelay);
            }
            setTimeout(scheduleNextComment, 30000);
        }
