...
function initCtaPact() {
    ...
    function initCommentSystem() {
        ...
        const notificationElement = document.getElementById('social-proof-notification');
        const caseStudiesContainer = document.getElementById('case-studies-container'); // Adicionado
        ...
        // (As funções auxiliares como generateUsername, allCommentsData, etc. permanecem aqui)
        const generateUsername = (name) => { const slugify = (text) => { return text.toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); }; const parts = name.split(' '); const firstName = slugify(parts[0]); const lastName = parts.length > 1 ? slugify(parts[parts.length - 1]) : ''; const patterns = [ (f, l) => l ? `${f}.${l.charAt(0)}` : f, (f, l) => `${f}${l}`, (f, l) => l ? `${f.charAt(0)}_${l}` : f, (f, l) => `${f}_${Math.floor(Math.random() * 90) + 10}` ]; return patterns[Math.floor(Math.random() * patterns.length)](firstName, lastName); };
        const allCommentsData = compradoresDB.map((comprador, index) => ({ id: 101 + index, username: generateUsername(comprador.name), text: commentTemplates[index % commentTemplates.length], originalTimestamp: new Date(Date.now() - ((2 + (index % 28)) * 24 * 3600 * 1000) - (Math.random() * 12 * 3600 * 1000)).toISOString(), initialLikes: 15 + Math.floor(Math.random() * 200) }));
        const staticComments = allCommentsData.slice(0, 30); const dynamicCommentPool = allCommentsData.slice(30);
        const simulateLikes = (base, ts) => { const hoursSincePost = (Date.now() - new Date(ts).getTime()) / (1000 * 3600); if (hoursSincePost < 24) { return Math.floor(hoursSincePost * (Math.random() * 5 + 1)); } return base + Math.floor(hoursSincePost * (Math.random() * 0.5 + 0.1)); };
        const formatTime = (ts) => { const seconds = Math.round((Date.now() - new Date(ts).getTime()) / 1000); if (seconds < 5) return 'agora mesmo'; const rtf = new Intl.RelativeTimeFormat('pt-BR', { numeric: 'auto' }); const intervals = [{ u: 'year', v: 31536000 }, { u: 'month', v: 2592000 }, { u: 'day', v: 86400 }, { u: 'hour', v: 3600 }, { u: 'minute', v: 60 }, { u: 'second', v: 1 }]; for (const i of intervals) { const amount = Math.floor(seconds / i.v); if (amount >= 1) return rtf.format(-amount, i.u); } return 'há poucos segundos'; };
        const createCommentEl = (comment) => { const el = template.content.cloneNode(true).querySelector('.mural-post'); const liked = (getFromStorage(LIKED_KEY) || {})[comment.id]; const likesCache = getFromStorage(CACHE_KEY) || {}; let likeCount = likesCache[comment.id] || simulateLikes(comment.initialLikes, comment.timestamp); if (liked && !likesCache[comment.id]) likeCount++; likesCache[comment.id] = likeCount; el.dataset.id = comment.id; el.querySelector('.post-username').textContent = comment.username; el.querySelector('.post-body p').textContent = `${comment.text}`; el.querySelector('.post-timestamp').textContent = formatTime(comment.timestamp); el.querySelector('.post-like-button .like-count').textContent = likeCount; if (liked) el.querySelector('.post-like-button').classList.add('is-liked'); el.querySelector('.post-avatar').innerHTML = `<img src="https://i.pravatar.cc/50/50?u=${comment.id}" alt="Avatar" loading="lazy">`; saveToStorage(CACHE_KEY, likesCache); return el; };
        const renderMural = (filter) => { listElement.innerHTML = ''; const seen = getFromStorage(SEEN_KEY) || {}; let commentsToDisplay; if (filter === 'relevantes') { commentsToDisplay = staticComments.map(c => ({ ...c, timestamp: c.originalTimestamp, likeCount: (getFromStorage(CACHE_KEY) || {})[c.id] || simulateLikes(c.initialLikes, c.originalTimestamp) })).sort((a, b) => b.likeCount - a.likeCount); } else { const impulseToRender = dynamicCommentPool.filter(c => seen[c.id]).map(c => ({...c, timestamp: seen[c.id]})); commentsToDisplay = impulseToRender.map(c => ({ ...c, likeCount: (getFromStorage(CACHE_KEY) || {})[c.id] || simulateLikes(c.initialLikes, c.timestamp) })).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); } if(commentsToDisplay.length === 0) { const message = filter === 'relevantes' ? "Nenhum testemunho encontrado." : "Nenhum comentário adicionado ainda. Fique de olho!"; listElement.innerHTML = `<p class="no-comments-message">${message}</p>`; } else { commentsToDisplay.forEach(c => { const el = createCommentEl(c); listElement.appendChild(el); setTimeout(() => el.classList.add('is-visible'), 50); }); } };
        ...
        caseStudiesContainer.style.display = 'none'; // Começa escondido
        listElement.style.display = 'block'; // Começa visível
        
        seedRecentComments();
        renderMural(activeFilter);
        scheduleNewComments();
        initCaseStudyToggle(); // Ativa a funcionalidade de toggle
