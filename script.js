...
const fakePurchases = [
    ...
    const allRelatos = [
        ...
        { username: 'Sérgio K.', text: 'O mais louco é que funciona com todas as idades. Apliquei os princípios com mulheres mais novas e mais velhas. A resposta do cérebro reptiliano delas é a mesma. É biologia, não tem como argumentar.'},
        { username: 'Pedro H.', text: 'Minha ex me viu numa festa outro dia... o jeito que ela me olhou, tentando entender o que tinha mudado... aquele olhar valeu cada centavo. Ela sentiu a diferença.'}
    ];
    ...
    function addPrimordialComment(post, isNew = false) {
        ...
        postDiv.className = 'mural-post'; // Usando a nova classe de estilo
        const timeAgo = isNew ? formatTimeAgo(new Date()) : formatTimeAgo(generateConsistentDate(post.username));
        postDiv.innerHTML = `
            <div class="post-header">
                <img src="img/avatar-placeholder.png" alt="Avatar" class="post-avatar">
    ...
    // Agenda a aparição dos comentários aleatórios
    function scheduleNextPost() {
        // Frequência bem reduzida para não competir com as notificações de compra
        const randomDelay = Math.random() * (60000 - 30000) + 30000; // Entre 45s e 90s
        
        setTimeout(() => {
            // Mostra até 5 novos comentários aleatórios
    ...
    setTimeout(scheduleNextPost, 30000);
}
